...
export default function UploadDropzone({ file, onFileChange }: Props) {
  ...
  const onDrop = useCallback(
    (accepted: File[], rejections: FileRejection[]) => {
      ...
      if (reason === "file-too-large") {
        ...
        setRejectionError("That file couldn't be uploaded. Please try a different one.");
      }
    },
    [onFileChange]
  );
  ...
  return (
    <div>
      ...
      {rejectionError && (
        <p style={{ marginTop: "10px", fontSize: "12px", color: "#f87171", textAlign: "center" }}>{rejectionError}</p>
      )}
    </div>
  );
}